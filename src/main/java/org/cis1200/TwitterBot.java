...
public class TwitterBot {
    ...
    public TwitterBot(BufferedReader br, int tweetColumn, NumberGenerator ng) {
        ...
        this.ng = ng;
        List<List<String>> sentences = TweetParser.csvDataToTrainingData(br, tweetColumn);
    ...
    public String generateTweet(int numWords) {
        ...
        String tweet = "";
        if (numWords > 0) {
            // if no start words or no sentences
            if (mc.startWords.getTotal() == 0 || mc.chain.size() == 0 || !mc.hasNext()) {
                return tweet;
            }
            tweet += mc.next();
