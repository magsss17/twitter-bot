...
public class TweetParser {
    ...
    static String extractColumn(String csvLine, int csvColumn) {
        ...
        try {
            String ret = sections[csvColumn];
        } catch (ArrayIndexOutOfBoundsException e) {
            return null;
        }
        return sections[csvColumn]; // Complete this method.
    }
    ...
    static List<String> csvDataToTweets(BufferedReader br, int tweetColumn) {
        ...
        // iterate through fl adding all the desired tweets
        while (fl.hasNext()) {
            String curr = fl.next();
            String tweet = extractColumn(curr, tweetColumn);
            if (tweet != null) {
                tweets.add(tweet);
            }
        }
    ...
    static List<List<String>> parseAndCleanTweet(String tweet) {
        String removed = removeURLs(tweet);
        List<String> sentences = tweetSplit(removed);
        List<List<String>> cleaned = new LinkedList<>();
        for (String sentence : sentences) {
            List<String> clean = parseAndCleanSentence(sentence);
            if (clean != null) {
                cleaned.add(clean);
            }
        }
    ...
    public static List<List<String>> csvDataToTrainingData(
        ...
        for (String tweet : tweets) {
            ...
            if (!cleaned.isEmpty() && cleaned != null && !cleaned.equals("")) {
                // add each sentence to the data
                // INVARIANT: cleaned only has non-null sentences
                for (List<String> sentence : cleaned) {
                    if (!sentence.isEmpty()) {
                        data.add(sentence);
                    }
                }
