...
public class MarkovChain implements Iterator<String> {
    ...
    void addBigram(String first, String second) {
        ...
        // first word already exists in dictionary
        if (chain.containsKey(first)) {
            // find the corresponding probability distribution and record word
            ProbabilityDistribution<String> pd = chain.get(first);
            pd.record(second);
            chain.put(first, pd);
        } else { // first word doesn't already exist in dictionary
            // add word and its distribution to the dictionary
    ...
    public void train(Iterator<String> sentence) {
        ...
        if (sentence.hasNext()) {
            ...
            // iterate through remaining sentence
    ...
    public void reset(String start) {
        ...
        if (!start.equals(END_TOKEN)) {
            next = start;
            startWords.record(start);
            if (!chain.containsKey(next)) {
                addBigram(next, END_TOKEN);
            }
            hasNext = true;
        } else {
            // next will be "<END>" when start is <END>
            next = END_TOKEN;
            hasNext = false;
        }
    ...
    public boolean hasNext() {
        // if (next != END_TOKEN && next != null) {
        // if (chain.containsKey(next)) {
        // ProbabilityDistribution<String> pd = chain.get(next);
        // boolean b = !(pd.getRecords().containsKey(END_TOKEN) && pd.getTotal() == 1);
        // if (b) {
        // return true;
        // }
        // }
    ...
    public String next() {
        ...
        if (!hasNext) {
            ...
            System.out.println(pd.getRecords());
            String nextWord = pd.pick(ng);
            ...
            // check if the current is at the end
            // ProbabilityDistribution<String> pd = chain.get(curr);
            // Map<String, Integer> rec = pd.getRecords();
            // boolean hasEnd = rec.containsKey(END_TOKEN);
            // if (hasEnd) {
            // if (pd.getTotal() == 1) {
            // hasNext = false;
            // next = null;
        ...
        // if (temp.equals(END_TOKEN)) {
        // throw new NoSuchElementException();
        // }
        // ProbabilityDistribution<String> pd = chain.get(temp);
        // next = pd.pick(ng);
        // if (next.equals(END_TOKEN)) {
        // hasNext = false;
        // } else {
        // hasNext = true;
        // }
        // return temp; // Complete this method.
    }
